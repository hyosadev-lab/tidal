import { buildAuthQuery, buildMessage, detectAlgorithm, sign } from "./signer.ts";

/**
 * OpenApiClient — GMGN OpenAPI client, ported from gmgn-skills/src/client/OpenApiClient.ts.
 *
 * Auth modes:
 *   Exist (market/token/portfolio): X-APIKEY + timestamp + client_id
 *   Signed (swap and order routes): the above + X-Signature (private key signature)
 *
 * Trimmed from the source CLI client: no package.json version lookup (no User-Agent
 * version pin), no `--raw`/CLI formatting concerns. Rate-limit retry-once and the
 * GMGN_DEBUG curl logging are kept as-is.
 */
...
export class OpenApiClient {
  ...
  async getTokenKline(chain: string, address: string, resolution: string, from?: number, to?: number): Promise<unknown> {
  ...
  async getWalletHoldings(chain: string, walletAddress: string, extra: Record<string, string | number> = {}): Promise<unknown> {
    return this.authSignedRequest("GET", "/v1/user/wallet_holdings", { chain, wallet_address: walletAddress, ...extra }, null);
  }
  ...
  async getWalletActivity(
    ...
    return this.authExistRequest("GET", "/v1/user/wallet_activity", { chain, wallet_address: walletAddress, ...extra });
  }
  ...
  async getTrendingSwaps(chain: string, interval: string, extra: Record<string, string | number | string[]> = {}): Promise<unknown> {
  ...
  private async doFetch(
    ...
    try {
      ...
      if (errorCode === "EADDRNOTAVAIL" || errorCode === "ENETUNREACH") {
        throw new Error(
          `Network unreachable (${errorCode}): Your system may not support IPv4. Please check your network configuration or contact support.`,
        );
      }
  ...
  private async parseResponse(method: string, path: string, res: Response, curlStr: string): Promise<unknown> {
...
function getRateLimitRetryDelayMs(err: unknown, attempt: number, maxAttempts: number, autoRetryOnRateLimit: boolean): number | null {
  if (!autoRetryOnRateLimit || attempt >= maxAttempts) return null;
  if (!(err instanceof OpenApiError)) return null;
  if (err.apiError !== "RATE_LIMIT_EXCEEDED" && err.apiError !== "RATE_LIMIT_BANNED") return null;
  if (err.resetAtUnix == null) return null;
...
function getAutoRetryMaxWaitMs(): number {
  ...
  if (!raw) return DEFAULT_RATE_LIMIT_AUTO_RETRY_MAX_WAIT_MS;
  const parsed = Number.parseInt(raw, 10);
...
function buildOpenApiErrorMessage(params: OpenApiErrorParams): string {
  ...
  if (params.status !== 429) return message;

  const resetText = params.resetAtUnix != null ? formatRateLimitReset(params.resetAtUnix) : "an unknown time";
...
const TRENCHES_PLATFORMS: Record<string, string[]> = {
  sol: [
    "Pump.fun", "pump_mayhem", "pump_mayhem_agent", "pump_agent", "letsbonk", "bonkers", "bags", "memoo", "liquid",
    "bankr", "zora", "surge", "anoncoin", "moonshot_app", "wendotdev", "heaven", "sugar", "token_mill", "believe",
    "trendsfun", "trends_fun", "jup_studio", "Moonshot", "boop", "ray_launchpad", "meteora_virtual_curve", "xstocks",
  ],
  bsc: [
    "fourmeme", "fourmeme_agent", "bn_fourmeme", "four_xmode_agent", "cubepeg", "likwid", "goplus_creator",
    "goplus_skills", "openfour", "flap", "flap_stocks", "flap_aioracle", "clanker", "lunafun",
  ],
  ...
  robinhood: ["noxa", "virtuals_v2", "bankr", "dyorswap", "pool_uniswap_v2", "pool_uniswap_v3", "pool_uniswap_v4"],
...
function buildTrenchesBody(
  ...
  // launchpad_platform / quote_address_type act as allow-list filters: sending an empty
  // array filters out every result. Configured chains supply real values; for any chain
  // missing config we omit the field so the API applies its own defaults instead of
  // returning an all-empty response.
...
function extractRootCause(err: unknown): unknown {
  if (err instanceof Error && err.cause) return extractRootCause(err.cause);
  return err;
}
