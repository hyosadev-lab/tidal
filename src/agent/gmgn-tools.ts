...
/**
 * One Tool per OpenApiClient endpoint (src/gmgn/endpoint.ts) — the full GMGN OpenAPI
 * surface, not just the read-only subset trading/engine.ts exposes to its analyst.
 * This file is consumed by src/cli.ts, the interactive human-in-the-loop chat, not by
 * the headless trading engine — see CLAUDE.md's "analystTools ... read-only by design"
 * note for why the engine's tool set stays separate and narrower.
 *
...
function tool(description: string, properties: Record<string, unknown>, required: string[], run: (a: any) => Promise<unknown>): Tool {
  return { description, parameters: { type: "object", properties, required }, run };
}

export const gmgnTools: Record<string, Tool> = {
  // ---- Token ----

  gmgn_token_info: tool(
  ...
  gmgn_wallet_token_balance: tool(
  ...
  gmgn_user_info: tool("Get the API key's own account info and bound wallets.", {}, [], () => gmgnClient().getUserInfo()),

  gmgn_follow_wallet: tool(
  ...
  gmgn_gas_price: tool("Get the current native-token gas price and USD price for a chain.", { chain }, ["chain"], ({ chain }) =>
    gmgnClient().getGasPrice(chain),
  ),

  gmgn_query_order: tool(
  ...
  gmgn_cooking_statistics: tool("Get platform-wide token-launch (\"cooking\") statistics.", {}, [], () =>
    gmgnClient().getCookingStatistics(),
  ),

  // ---- Trade (spends real funds — refuses unless GMGN_ALLOW_AUTOMATED_TRADES=1) ----
